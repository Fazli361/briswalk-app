      ...
      <div className="max-w-4xl mx-auto px-4 py-3 sm:px-6 sm:py-4 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          <div className="w-14 h-14 sm:w-16 sm:h-16 shrink-0 bg-white rounded-xl border border-slate-100 p-1 shadow-xs flex items-center justify-center overflow-hidden">
            <img
              src="https://images.seeklogo.com/logo-png/47/1/panel-penasihat-klinik-kesihatan-logo-png_seeklogo-470691.png"
              alt="Logo Panel Penasihat Klinik Kesihatan"
              className="w-full h-full object-contain"
              referrerPolicy="no-referrer"
              onError={(e) => {
                // Graceful fallback if image is blocked
                const target = e.currentTarget;
                target.style.display = 'none';
                if (target.parentElement) {
                  target.parentElement.innerHTML = '<span class="text-xs font-bold text-emerald-700 text-center">PPKK</span>';
                }
              }}
            />
          </div>
